...
const CollectionsForm = ({ collection, updateUser, cancel, debug, apiUrl, ...props }) => {
  ...
  const validate = (values) => {
    ...
    if (
      ...
      !/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(values.extras.contactEmail)
    ) {
      ...
      render={({
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <WrappedField
            ...
            required='true'
          />
          <div className='mt-gutter bg-yellow-300 p-4 m-4 border-dotted border-4' hidden={!debug}>
            <Field
              className='mr-4'
              name='extras.master'
              type='checkbox'
          ...
          <WrappedField label='Program Code' name='extras.programCode' type='string' />
          <WrappedField
            label='Data level'
          ...
          <WrappedField label='Organization' name='extras.organization' type='string' />
          <WrappedField
          ...
          <WrappedField label='Frequency' name='extras.frequency' type='string' />
          <WrappedField
            label='Contact Name'
          ...
          <WrappedField
            ...
            required={true}
          />
          <WrappedField label='Home Page' name='extras.homePageUrl' type='string' />
          <WrappedField label='References' name='extras.references' type='string' />
          {values.extras && values.extras.master && (
            <div>
              <h3 className='block text-gray-600 mt-gutter mb-2 ml-4'>Add to Master Collection</h3>
              <AutocompleteFetch
                values={values}
                apiUrl={apiUrl}
                name='groups'
          ...
          {(!values.extras || !values.extras.master) && (
            <div>
              <h3 className='block text-gray-600 mt-gutter mb-2 ml-4'>Add Data Profiles</h3>
              <AutocompleteFetch
                values={values}
                apiUrl={apiUrl}
                name='packages'
          ...
          {debug && <Debug vals={{ dirty, values, errors, touched, isSubmitting, status }} />}
        </Form>
      )}
    />
  )
